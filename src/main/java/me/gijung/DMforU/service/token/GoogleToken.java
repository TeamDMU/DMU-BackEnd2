...
public class GoogleToken implements Token<ServiceTokensDto> {
    ...
    public void updateToken(ServiceTokensDto tokensDto) throws FirebaseMessagingException {
    ...
    private void firebaseSendToken(ServiceTokensDto tokensDto, BiConsumer<List<String>, Topic> tokenProcesss) {
        EnumSet<Topic> topics = EnumSet.allOf(Topic.class);
        List<Topic> topic1 = tokensDto.getTopic();
        for (Topic topic : topics) {
            if (topic1.contains(topic)) {
                tokenProcesss.accept(Collections.singletonList(tokensDto.getTokens()), topic);
            }
        }
    }

    private synchronized void deleteAllTopic(ServiceTokensDto tokensDto) throws FirebaseMessagingException {
        for (Topic value : Topic.values()) {
            FirebaseMessaging.getInstance().unsubscribeFromTopic(Collections.singletonList(tokensDto.getTokens()), String.valueOf(value));
        }
    }
}
